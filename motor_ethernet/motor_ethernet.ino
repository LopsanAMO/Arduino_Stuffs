...
IPAddress ip(192,168,1,89l); //Asingamos la IP al Arduino
  ...
  if (client) {
      ...
      if (client.available()) {
        ...
        cadena.concat(c);//Unimos el String 'cadena' con la petición HTTP (c). De esta manera convertimos la petición HTTP a un String
         ...
         int posicion=cadena.indexOf("LED="); //Guardamos la posición de la instancia "LED=" a la variable 'posicion'
          ...
          if(cadena.substring(posicion)=="LED=IZQ")//Si a la posición 'posicion' hay "LED=OFF"
          {
            digitalWrite(led,LOW);
               digitalWrite(c1,LOW);
          ...
          if(cadena.substring(posicion)=="LED=DER")//Si a la posición 'posicion' hay "LED=OFF"
          {
            digitalWrite(led,HIGH);
   ...
   digitalWrite(c2,LOW);
        ...
        if (c == '\n' && currentLineIsBlank) {
            ...
            client.println("OFF");
            client.println("</button>");
            client.println("<button onClick=location.href='./?LED=DER\' style='margin:auto;background-color: #84B1FF;color: snow;padding: 10px;border: 1px solid #3F7CFF;width:65px;'>");
            client.println("DER");
            client.println("</button>");
            client.println("<button onClick=location.href='./?LED=IZQ\' style='margin:auto;background-color: #84B1FF;color: snow;padding: 10px;border: 1px solid #3F7CFF;width:65px;'>");
            client.println("IZQ");
            client.println("</button>");
