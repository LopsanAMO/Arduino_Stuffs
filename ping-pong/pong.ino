...
void processInputs() {
 ...
 //  button2Status = (digitalRead(BUTTON_TWO_PIN) == LOW);
   ...
   {
     Serial.println("Perdiste shavo,LLegale");
     drawMenu ();
   }
...
void playerScored(byte player) {
 ...
 void drawBox() {
  ...
  // had to make box a bit smaller to fit tv
    TV.draw_line(0, 0, 0,95,2 );  // left
   TV.draw_line(0, 0, 126,0,2 ); // top
    TV.draw_line(126, 0, 126,95,2 ); // right
     TV.draw_line(0, 95, 126,95,2 ); // bottom
...
void drawMenu() {
  ...
  TV.print(10, 5, "Superduper Amazing inmaculate Arduino pong of the good proyects ");
  TV.select_font(font4x6);
  TV.print(22, 35, "Puchale");
  TV.print(30, 45, "al Start");
...
//    if(button1Status) Serial.println((int)ballVolX);
  ...
  if(state == GAME_OVER) {
    drawGameScreen();
    TV.select_font(font8x8);
    TV.print(29,25,"Perdiste");
    TV.print(68,25,"shavot");
    while(!button1Status) {
      processInputs();
      delay(50);
    }
